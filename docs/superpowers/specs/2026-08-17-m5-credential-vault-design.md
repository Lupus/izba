...
- **F-CRED-5 (MED, prerequisite) — CLOSED ([#230](https://github.com/Lupus/izba/issues/230)).** Full register entry: [F-CRED-5 in the findings register](../../security/findings-2026-06-15.md) (this bullet is the design-side pointer; the register is the authority). The egress accept loop now runs `peercred::authorize_stream` on every connection, before any per-connection work and before a handler thread exists; a foreign-uid peer is logged (`egress_admission`, whose `Reject` line names the sandbox) and dropped, so it sees EOF and relays no bytes. Linux: enforced via `SO_PEERCRED`. Windows: unchanged residual, identical to F-09 — no peer-credential API, so the verdict is `Allow(Unavailable)`; izba never *hardens* the socket there (the run-dir hardening is `cfg(unix)`), so it inherits the `%LOCALAPPDATA%` profile DACL, and note izba does author that dir's security descriptor on Windows but only in the widening direction (a default-on Low integrity label so the Low-IL VMM can write, plus an `izba-sb-<name>` Modify ACE under `izba lockdown`). The daemon's startup line now reports that posture for *both* planes rather than naming only the control socket, and names the still-uncovered USB broker plane so the report is not read as a complete inventory.

    **The P2 prerequisite is satisfied ON LINUX ONLY — read this before attaching real credential material.** The gate P2 was told to wait for does not exist on Windows and cannot be built from `SO_PEERCRED` there. A Windows P2 that injects real secrets into a flow would authenticate no peer at all: any local process able to open `<run dir>/vsock.sock_1027` would drive a credential-bearing flow, bounded only by an inherited profile DACL that izba does not author. P2 therefore needs its own Windows answer — an explicit DACL on the run dir naming only the VMM principal, a per-sandbox account (`izba lockdown`) made mandatory for credential-bearing sandboxes, or a refusal to attach real material on Windows until one of those lands. Do not read "F-CRED-5 CLOSED" as clearing P2 on both platforms. Spike [#248](https://github.com/Lupus/izba/issues/248) (2026-09-03) then sharpened the Windows gap: AF_UNIX `connect()` *requires* write access on the socket file, so the run-dir Low label the Low-IL VMM needs to dial `vsock.sock_1027` also admits every other same-user Low-IL process, including sibling sandboxes' VMMs — the "explicit DACL naming the VMM principal" option above is now a concrete accept-time gate tracked as [#276](https://github.com/Lupus/izba/issues/276).

    Note also what the gate does **not** establish on either platform: it proves *"same uid as izbad"*, not *"is the VMM"*, so a same-user process still passes. Credential material reachable by driving this plane is therefore exposed to any same-user compromise — the same limitation F-CRED-3 records for the credential store itself. Original finding, for the record: `crates/izba-core/src/daemon/egress/mod.rs:248` accepted connections on the per-sandbox vsock-1027 unix listener (`<run dir>/vsock.sock_1027`) with **no peer check**, gated only by the same 0700 run directory that F-09 already established is insufficient as the *sole* gate for a peer-authoritative socket. This is precisely the plane P2 will use to inject real credential material into an outbound request, and its only legitimate peer is the same-uid VMM process bridging the guest's vsock dial — exactly the shape `peercred::authorize_stream` (F-09, `crates/izba-core/src/daemon/peercred.rs`) already solves for the control socket. Before any grant can attach real secrets to a flow, the same accept-time check had to be wired onto this listener. Named here so it was caught in review rather than rediscovered once the vault exists.
