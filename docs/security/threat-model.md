...
| **B3** | T4 guest → T1 izbad egress plane (vsock 1027) | `StreamOpen::{TcpConnect,Dns}` frames, then TLS/HTTP | The *live* hostile-guest→host boundary. izbad is an outbound proxy the guest drives → SSRF, policy bypass, DNS exfil (F-01..F-05). The listener's *local-peer* side (a T3 local user opening `<run dir>/vsock.sock_1027` directly, rather than a guest arriving through the VMM bridge) is gated by the same accept-time peer-uid check as **B2** — F-CRED-5 closed on Linux via `SO_PEERCRED`; on Windows the residual is identical to F-09's — izba never *hardens* the socket's permissions there (the 0700 run-dir hardening is `cfg(unix)`), so it inherits the `%LOCALAPPDATA%` profile DACL. Note izba does author that directory's security descriptor on Windows, but only in the *widening* direction: a default-on inheritable Low integrity label (`VmSpec::confined_write_surfaces`, so the Low-IL VMM can write) and, under `izba lockdown`, a Modify ACE for the per-sandbox `izba-sb-<name>` account (`compute_grants`). Spike #248 (2026-09-03, [`docs/spikes/0001`](../spikes/0001-windows-afunix-connect-write-access.md)) established that this widening is live: Windows AF_UNIX `connect()` requires write access on the socket file, so the Low label is both *required* by the Low-IL VMM's own dial and *sufficient* for any other same-user Low-IL process — including a sibling sandbox's VMM — to connect. Accept-time gate to the sandbox's own VMM: #276. |
