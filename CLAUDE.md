...
| [docs/credential-proxying-building-blocks.md](docs/credential-proxying-building-blocks.md) | **Credential-proxying survey + M5 design space** — the four credential families (bearer / OAuth-minted / signature-based / non-HTTP), the downscope→inject→sentinel→passthrough ladder, prior art (Docker `sbx`, OpenShell providers-v2, Secretless, SPIFFE, virtual-key gateways), and the failure list. Read before M5 vault work. |
...
- `app/src-tauri` (Tauri 2 GUI) — **OUT of the workspace, separate gate.** It is
  ...
  `npm run test` runs the Vitest unit + browser projects and needs Chromium
  ...
  Playwright IPC mock `app/e2e/mock/tauri-mock.js` —
  `app/src/test/tauriMockParity.test.ts` (part of `npm run test`) fails on a
  registered command with no mock case, on a stale mock case, and on a parse
  that finds implausibly few commands; deliberate exclusions go in its
...
- **SSH access (`ssh izba-<name>`):** a vendored static OpenSSH `sshd`
  ...
  image. root's login shell is `/init` (izba-init itself), which acts as a
  restricted login shell — no `ForceCommand`, no `ChrootDirectory`. OpenSSH
  invokes it as `-init` (dash-prefixed argv[0]) for an interactive login or as
  `/init -c "<cmd>"` for a remote command; izba-init detects both forms and routes
  either into `crun exec`, entering the running `izba` container exactly like
  `izba exec`. sftp-in-container is a follow-up.
