...
# izba fork: upstream bootstrapped apt-fast here via an unpinned
# `curl -sL .../quick-install.sh | bash` (remote code execution at CI runtime)
# to parallelize downloads with aria2c. We use plain apt-get instead: no
# unpinned remote code, and the downloads now honor the Acquire::Retries +
# Acquire::*::Timeout config the workflow writes to apt.conf.d. This path only
# runs on a cache MISS; cache hits restore via tar and never reach here, so the
# lost cold-path parallelism is a negligible, infrequent cost.
...
sudo DEBIAN_FRONTEND=noninteractive apt-get --yes install ${packages} > "${install_log_filepath}"
log "done"
